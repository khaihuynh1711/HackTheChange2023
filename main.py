...
def output(address, yearly_power_use, cost_of_power, roof_size):
    ...
    return [str(coordinates[0]) + " " + str(coordinates[1]), str(math.ceil(solar_result[0])), str(solar_result[1]),
            f'{total_cost:.2f}', f'{time:.2f}', f'{excess_money_per_year:.2f}']
...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.logo_label = customtkinter.CTkLabel(self.sidebar_frame, text="SOLAR SENSE YYC",
                                                 font=customtkinter.CTkFont(size=20, weight="bold"))
        self.logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))
        self.sidebar_button_1 = customtkinter.CTkButton(self.sidebar_frame, text="About us",
                                                        command=self.about_us_button)
        self.sidebar_button_1.grid(row=1, column=0, padx=20, pady=10)
        self.sidebar_button_2 = customtkinter.CTkButton(self.sidebar_frame, text="Application",
                                                        command=self.application_button)
        self.sidebar_button_2.grid(row=2, column=0, padx=20, pady=10)
        self.exit_button = customtkinter.CTkButton(self.sidebar_frame, text="Exit", command=self.exit_program,
                                                   fg_color="#8B0000")
    ...
    def application_button(self):
        ...
        self.cost_entry = customtkinter.CTkEntry(self.slider_progressbar_frame,
                                                 placeholder_text="Numeric value for price (CAD/Kwh)")
    ...
    def generate_data(self):
        ...
        final_info = output(address_entry_state, float(power_entry_state), float(cost_entry_state),
                            float(roof_entry_state))
        print(final_info)
        ...
        if checkbox_2_state == 1:
            ...
            self.power = customtkinter.CTkLabel(self.tabview.tab("Power Generated"),
                                                text="Average household power usage per year: " + power_entry_state +
                                                     " Kwh\nTotal power generated by solar panel per year: " +
                                                     final_info[2] + " Kwh\n\nExcess power: " + str(
                                                    float(final_info[2]) - float(power_entry_state)) + " Kwh",
                                                justify="left",
                                                font=("Helvetica", 20))
        ...
        if checkbox_4_state == 1:
            ...
            if float(final_info[5]) >= 0:
                self.finance = customtkinter.CTkLabel(self.tabview.tab("Financial"),
                                                      text="You earned $" + final_info[
                                                          5] + " through the grid per year.",
                                                      justify="left",
                                                      font=("Helvetica", 20))
        ...
        self.custom_frame_next_to_checkbox = customtkinter.CTkFrame(self, width=100, height=100, bg_color="transparent",
                                                                    fg_color="#8B0000")
        self.custom_frame_next_to_checkbox.grid(row=1, column=2, padx=(345, 10), pady=(10, 0), sticky="nsew",
                                                columnspan=2)
